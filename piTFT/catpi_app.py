...
class CatpiApp(App):
    ...
    # def on_auto_is_on(self, instance, value):
    #     if self._updating_ui:
    #         return
    #     if self._publish_clock is None:
    #         self._publish_clock = Clock.schedule_once(
    ...
    def toggle_auto_mode(self):
        self.auto_is_on = not self.auto_is_on
        self._update_device()

    def save_times(self, breakfast_time, lunch_time, dinner_time):
        self.breakfast = breakfast_time
        self.lunch = lunch_time
        self.dinner = dinner_time
        self._update_device()
    ...
    def _update_ui(self, new_state):
        ...
        try:
            ...
            if 'auto' in new_state:
                self.auto_is_on = new_state['auto']
            if 'settings' in new_state:
                self.breakfast = new_state['settings']['breakfast']
                self.lunch = new_state['settings']['lunch']
                self.dinner = new_state['settings']['dinner']
        finally:
    ...
    def _update_device(self):
        msg = {'remaining': self._remaining,
               'auto': self.auto_is_on,
               'settings': {'breakfast': self._breakfast,'lunch': self._lunch,'dinner': self._dinner},
               'client': MQTT_CLIENT_ID}
